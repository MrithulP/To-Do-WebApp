document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTask(task) {
        ...
        const difficultyColors = {
            easy: '#69F0AE', // Green for easy
            medium: '#FFAB40', // Orange for medium
            hard: '#FF5252' // Red for hard
        };

        taskCard.innerHTML = `
            <h3>${task.taskName}</h3>
            <p><strong>Importance:</strong> <span class="importance-${task.importance.toLowerCase()}">${task.importance}</span></p>
            <p><strong>Difficulty:</strong> <span class="difficulty-${task.difficulty.toLowerCase()}" style="color: ${difficultyColors[task.difficulty.toLowerCase()]};">${task.difficulty}</span></p>
    ...
    function updateCounts() {
        ...
        document.getElementById('completed-count').innerText = completedCount; // Show how many tasks are completed in the task list
        document.getElementById('total-completed-count').innerText = completedCountAllTime; // Show the total number of tasks completed
